...
#endif
	// 投票矩阵的宽高深
	unsigned int votew = w; // w
	unsigned int voteh = h; // h
	unsigned int voted = sqrt(votew*votew+voteh*voteh);
	vote = Mat(votew, voteh, voted, 1, 0); // 投票矩阵，其中宽高深是调整效果的要点。
	double rmin = 20;// 太小的圆视为孤立点或噪声干扰，不能当成结果
