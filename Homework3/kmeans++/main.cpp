...
typedef struct PP { double x, y; int group; } point_t, *point;
...
{
	double ang, r;
	point p, pt = (PP*)malloc(sizeof(point_t) * count);
...
point lloyd(point pts, int len, int n_cluster)
{
	...
	point cent = (PP*)malloc(sizeof(point_t) * n_cluster), p, c;
